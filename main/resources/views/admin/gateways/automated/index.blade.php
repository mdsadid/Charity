                ...
                <div class="card h-100 text-center">
                    <div class="card-body px-3">
                        ...
                        <div class="d-flex gap-2 flex-wrap justify-content-center">
                            <a href="{{ route('admin.gateway.automated.edit', $gateway->alias) }}" class="btn btn-label-primary">
                                <span class="tf-icons las la-pen me-1"></span> @lang('Edit')
                            </a>

                            @if ($gateway->status)
                                <button class="btn btn-label-warning decisionBtn" data-question="@lang('Are you confirming the inactivation of this gateway?')" data-action="{{ route('admin.gateway.automated.status', $gateway->id) }}">
                                    <span class="tf-icons las la-ban me-1"></span>
                                    @lang('Inactive')
                                </button>
                            @else
                                <button class="btn btn-label-success decisionBtn" data-question="@lang('Are you confirming the activation of this gateway?')" data-action="{{ route('admin.gateway.automated.status', $gateway->id) }}">
