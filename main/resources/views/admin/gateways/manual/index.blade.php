                ...
                <div class="card h-100 text-center">
                    <div class="card-body px-3">
                        ...
                        <div class="d-flex gap-2 flex-wrap justify-content-center">
                            <a href="{{ route('admin.gateway.manual.edit', $method->id) }}" class="btn btn-label-primary">
                                <span class="tf-icons las la-pen me-1"></span> @lang('Edit')
                            </a>

                            @if ($method->status)
                                <button class="btn btn-label-warning decisionBtn" data-question="@lang('Are you confirming the inactivation of this method?')" data-action="{{ route('admin.gateway.manual.status', $method->id) }}">
                                    <span class="tf-icons las la-ban me-1"></span>
                                    @lang('Inactive')
                                </button>
                            @else
                                <button class="btn btn-label-success decisionBtn" data-question="@lang('Are you confirming the activation of this method?')" data-action="{{ route('admin.gateway.manual.status', $method->id) }}">
...
@push('breadcrumb')
    <a href="{{ route('admin.gateway.manual.new') }}" class="btn btn-label-primary">
